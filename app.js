...
let file1Data = null; // { trackpoints, stats, name, sport, startDate }
let file2RawData = null; // Unveränderte Quelldaten von Datei 2
let file2Data = null; // Angepasste Daten von Datei 2

let activePrimaryMetric = 'hr';
let activeSecondaryMetric = 'none';

// Farbkonfiguration
const COLOR_F1 = '#FFFFFF'; // Weiß
const COLOR_F2 = '#FF9800'; // Orange

const METRICS = {
    hr: { label: 'Puls (bpm)', reverseY: false },
    pace: { label: 'Pace (min/km)', reverseY: true },
    speed: { label: 'Geschwindigkeit (km/h)', reverseY: false },
    power: { label: 'Leistung (W, 30s Ø)', reverseY: false },
    gap: { label: 'GAP (min/km)', reverseY: true },
    alt: { label: 'Höhe (m)', reverseY: false },
    cadence: { label: 'Cadence (spm)', reverseY: false },
    eff: { label: 'Effizienz ((m/s / HR) * 1000)', reverseY: false }
};

// Event-Listener Dateiauswahl
...
// Event-Listener TCX 2 Menü Toggle
const tcx2SettingsBtn = document.getElementById('tcx2SettingsBtn');
const tcx2Menu = document.getElementById('tcx2Menu');

tcx2SettingsBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    tcx2Menu.classList.toggle('hidden');
});
